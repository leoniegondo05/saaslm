...
// Icône globe (sélecteur de langue), affichée en bas du footer à côté du copyright.
function GlobeIcon() {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
...
// TODO: remplacer "#" par les vraies URLs des réseaux sociaux une fois connues.
const SOCIALS = [
      ...
      <div className="mx-auto mt-16 flex max-w-[1320px] flex-col items-start justify-between gap-6 border-t border-black/10 pt-6 text-xs text-brand-bg/50 sm:flex-row sm:items-center sm:gap-4">
        ...
        <div className="order-2 flex items-center gap-2 sm:order-1">
          ...
          <button
            ...
            className="ml-1 flex h-7 w-7 items-center justify-center text-brand-bg/50 transition-opacity hover:opacity-70"
          >
            <GlobeIcon />
          </button>
        </div>

        {/* Icônes réseaux sociaux, chaque logo garde sa forme officielle (comme la maquette) */}
        <div className="order-3 hidden gap-3 sm:flex">
          {SOCIALS.map(({ name, href, Icon }) => (
            <a
              ...
              aria-label={name}
              className="flex h-7 w-7 items-center justify-center text-[#2E1A8E] transition-opacity hover:opacity-70"
            >
              <Icon />
            </a>
          ))}
