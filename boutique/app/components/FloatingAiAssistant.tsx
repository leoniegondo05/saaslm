...
const INTRO_MESSAGE_FR =
  "Bonjour Awa 👋 Je suis votre assistant LM. Dites-moi ce que vous voulez savoir sur votre boutique (ventes, commandes, recommandations...).";
const INTRO_MESSAGE_EN =
  "Hello Awa 👋 I'm your LM assistant. Tell me what you'd like to know about your shop (sales, orders, recommendations...).";
...
export default function FloatingAiAssistant() {
  ...
  // Message d'intro non stocké dans l'état : recalculé via t() à chaque
  // rendu (voir plus bas) pour rester dans la bonne langue même si elle
  // change après le montage, plutôt qu'un texte figé dans messages[0].
  ...
  const sendMessage = (text: string) => {
    ...
    setMessages((current) => [
      ...
      {
        role: "assistant",
        text: t("Analyse en cours... cette réponse sera bientôt générée par l'IA LM.", "Analyzing... this reply will soon be generated by LM AI."),
      },
    ]);
    setDraft("");
  };
  ...
  return (
    <>
      ...
      <button
        ref={bubbleRef}
        type="button"
        aria-label={open ? t("Fermer l'assistant IA", "Close the AI assistant") : t("Ouvrir l'assistant IA", "Open the AI assistant")}
        onMouseDown={(event) => {
          event.preventDefault();
      ...
      {open && (
        ...
        >
          ...
          <div className="flex items-center gap-3 border-b border-white/10 px-4 py-3.5">
            ...
            <div className="min-w-0 flex-1 leading-tight">
              <p className="truncate text-sm font-semibold text-white">
                {t("Analyser ma boutique", "Analyze my shop")}
              </p>
              <p className="text-[11px] font-medium tracking-wide text-white/50">
                {t("ASSISTANCE LM", "LM SUPPORT")}
              </p>
            </div>
            <button
              type="button"
              aria-label={t("Fermer l'assistant", "Close the assistant")}
          ...
          <div className="flex min-h-0 flex-1 max-h-[50vh] flex-col gap-2.5 overflow-y-auto px-4 py-4">
            {[{ role: "assistant" as const, text: t(INTRO_MESSAGE_FR, INTRO_MESSAGE_EN) }, ...messages].map((message, index) => (
              <p
                key={index}
